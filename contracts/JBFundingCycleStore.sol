...
contract JBFundingCycleStore is JBControllerUtility, IJBFundingCycleStore {
  ...
  */
		...
		returns (JBFundingCycle memory fundingCycle)
	{
		...
		if (_fundingCycleConfiguration > 0) {
			...
			if (
				!_isApproved(_projectId, _fundingCycle) ||
  ...
  */
		...
		returns (JBBallotState)
	{
		...
		JBFundingCycle memory _fundingCycle = _getStructFor(
			_projectId,
			_fundingCycleConfiguration
		);
  ...
  */
	function _configureIntrinsicProperiesFor(
		...
		if (!_isApproved(_projectId, _fundingCycle))
			...
			// which carries the last approved configuration.
			_currentConfiguration = _getStructFor(
				_projectId,
				_currentConfiguration
			).basedOn;
...
*/
  ...
  */
	function _standbyOf(uint256 _projectId)
		private
		view
  ...
  */
		...
		returns (uint256 configuration)
	{
		...
		if (block.timestamp >= _fundingCycle.start)
  ...
  */
		...
		returns (JBFundingCycle memory fundingCycle)
	{
		...
		fundingCycle.ballot = IJBFundingCycleBallot(
			address(uint160(_packedUserProperties))
		);
		fundingCycle.duration = uint256(uint64(_packedUserProperties >> 160));
		fundingCycle.discountRate = uint256(
			uint32(_packedUserProperties >> 224)
		);
  ...
  */
	function _ballotStateOf(
		...
		if (_ballotFundingCycle.ballot == IJBFundingCycleBallot(address(0)))
			...
			_ballotFundingCycle.ballot.duration() >=
			block.timestamp - _configuration
