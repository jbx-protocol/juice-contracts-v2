    ...
    ReentrancyGuard
{
  ...
  */
    function ethBalanceOf(uint256 _projectId)
        external
        view
        override
        returns (uint256)
  ...
  */
    function heldFeesOf(uint256 _projectId)
        external
        view
        override
  ...
  */
        ...
        )
    {
        ...
        (JBFundingCycle memory _fundingCycle, uint256 _withdrawnAmount) = store
  ...
  */
    function _takeFeeFrom(
        ...
        uint256 _discountedFee = fee -
            PRBMath.mulDiv(fee, _feeDiscount, JBConstants.MAX_FEE_DISCOUNT);

        // The amount of ETH from the _amount to pay as a fee.
        feeAmount =
            _amount -
            PRBMath.mulDiv(_amount, 200, _discountedFee + 200);
        ...
        _fundingCycle.shouldHoldFees()
            ? _heldFeesOf[_projectId].push(
                JBFee(_amount, uint8(fee), _beneficiary)
            )
  ...
  */
    function _pay(
        ...
        (_fundingCycle, _weight, _tokenCount, _memo) = store.recordPaymentFrom(
            ...
            _amount,
            _projectId,
            (_preferClaimedTokens ? 1 : 0) |
                (uint256(uint160(_beneficiary)) << 1),
            _minReturnedTokens,
            _memo,
            _delegateMetadata
        );
