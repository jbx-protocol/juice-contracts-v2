...
contract JBTokenStore is JBControllerUtility, JBOperatable, IJBTokenStore {
  ...
  */
        ...
        returns (uint256 supply)
    {
        ...
        if (_token != IJBToken(address(0)))
            supply = supply + _token.totalSupply(_projectId);
    }
  ...
  */
        ...
        returns (uint256 balance)
    {
        ...
        if (_token != IJBToken(address(0)))
            balance = balance + _token.balanceOf(_projectId, _holder);
    }
  ...
  */
    function mintFor(
        ...
        bool _shouldClaimTokens = (requireClaimFor[_projectId] ||
            _preferClaimedTokens) && _token != IJBToken(address(0));
  ...
  */
    function burnFrom(
        ...
        if (
            ...
            (_amount < _claimedBalance ||
                _unclaimedBalance < _amount - _claimedBalance) &&
            (_amount < _unclaimedBalance ||
                _claimedBalance < _amount - _unclaimedBalance)
        ) {
        ...
        if (_claimedBalance == 0) {
            ...
            _claimedTokensToBurn = _claimedBalance >= _amount
                ? _amount
                : _claimedBalance;
            // Otherwise, redeem unclaimed tokens before claimed tokens.
        } else {
            _claimedTokensToBurn = _unclaimedBalance >= _amount
                ? 0
                : _amount - _unclaimedBalance;
        }
        ...
        // burn the tokens.
        if (_claimedTokensToBurn > 0)
            _token.burn(_projectId, _holder, _claimedTokensToBurn);
