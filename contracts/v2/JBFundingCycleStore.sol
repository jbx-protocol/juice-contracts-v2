...
contract JBFundingCycleStore is JBTerminalUtility, IJBFundingCycleStore {
        ...
        if (_baseFundingCycle.duration == 0)
            return
                PRBMath.mulDiv(
                    ...
                    400 - _baseFundingCycle.discountRate,
                    400
                );
        ...
        if (_limitLength == 0 || _limitLength > _startDistance) {
            ...
            if (_latestPermanentFundingCycle.discountRate > 0) {
                ...
                for (uint256 i = 0; i < _permanentDiscountMultiple; i++) {
                    ...
                    weight = PRBMath.mulDiv(
                        weight,
                        400 - _latestPermanentFundingCycle.discountRate,
                        400
                    );
                }
            }
    ...
    */
    function _idFor(uint256 _projectId, uint256 _number)
        private
        pure
        returns (uint256)
    {
