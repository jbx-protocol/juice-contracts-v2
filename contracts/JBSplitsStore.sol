...
*/
  ...
  */
        ...
        )
    {
        ...
        JBSplit[] memory _currentSplits = _getStructsFor(
            _projectId,
            _domain,
            _group
        );
        ...
        for (uint256 _i = 0; _i < _splits.length; _i++) {
            ...
            _packedSplitParts1 |=
                uint256(uint160(address(_splits[_i].beneficiary))) <<
                89;

            // Store the first spit part.
            _packedSplitParts1Of[_projectId][_domain][_group][
                _i
            ] = _packedSplitParts1;
            ...
            if (
                ...
                _packedSplitParts2 |=
                    uint256(uint160(address(_splits[_i].allocator))) <<
                    48;
                _packedSplitParts2Of[_projectId][_domain][_group][
                    _i
                ] = _packedSplitParts2;
                // Otherwise if there's a value stored in the indexed position, delete it.
            } else if (
                _packedSplitParts2Of[_projectId][_domain][_group][_i] > 0
            ) {
  ...
  */
    function _getStructsFor(
        ...
        for (uint256 _i = 0; _i < _splitCount; _i++) {
            ...
            uint256 _packedSplitPart1 = _packedSplitParts1Of[_projectId][
                _domain
            ][_group][_i];
            ...
            _split.beneficiary = payable(
                address(uint160(_packedSplitPart1 >> 89))
            );

            // Get a reference to the second packed data.
            uint256 _packedSplitPart2 = _packedSplitParts2Of[_projectId][
                _domain
            ][_group][_i];
            ...
            if (_packedSplitPart2 > 0) {
                ...
                _split.allocator = IJBSplitAllocator(
                    address(uint160(_packedSplitPart2 >> 48))
                );
            }
