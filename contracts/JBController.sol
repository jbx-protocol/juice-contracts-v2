...
contract JBController is IJBController, JBOperatable, ReentrancyGuard {
  ...
  */
	function launchProjectFor(
		...
		if (_terminals.length > 0)
			directory.addTerminalsOf(projectId, _terminals);
	}
  ...
  */
		...
		returns (uint256 configuration)
	{
		...
		// Add the provided terminals to the list of terminals.
		if (_terminals.length > 0)
			directory.addTerminalsOf(_projectId, _terminals);
	}
  ...
  */
	function prepForMigrationOf(uint256 _projectId, IJBController)
		external
		override
	{
  ...
  */
	function _reservedTokenAmountFrom(
		...
		if (_reservedRate == JBConstants.MAX_RESERVED_RATE)
			return _unprocessedTokenBalanceOf;
	...
	function _configure(
		...
		for (uint256 _i; _i < _fundAccessConstraints.length; _i++) {
			JBFundAccessConstraints
				memory _constraints = _fundAccessConstraints[_i];

			// If distribution limit values are too large then revert.
			if (_constraints.distributionLimit > type(uint248).max)
				revert BAD_DISTRIBUTION_LIMIT();
			...
			if (_constraints.overflowAllowance > type(uint248).max)
				revert BAD_OVERFLOW_ALLOWANCE();
			...
			if (_constraints.distributionLimit > 0) {
				_packedDistributionLimitDataOf[_projectId][
					_fundingCycle.configuration
			...
			if (_constraints.overflowAllowance > 0) {
				_packedOverflowAllowanceDataOf[_projectId][
					_fundingCycle.configuration
				][_constraints.terminal] =
