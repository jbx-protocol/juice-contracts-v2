...
describe('JBController::mintTokensOf(...)', function () {
  ...
  async function setup() {
    ...
    let jbControllerFactory = await ethers.getContractFactory('contracts/JBController/2.sol:JBController');
    ...
    await expect(
      jbController
        ...
        .mintTokensOf(
          ...
          /* _useReservedRateOption=*/ USE_FUNDING_CYCLE,
        ),
    )
  ...
  it(`Should add the minted amount to the reserved tokens if _useReservedRateOption is set to reserved token only`, async function () {
    const {
      projectOwner,
      beneficiary,
      jbController,
    ...
    await mockJbFundingCycleStore.mock.currentOf.withArgs(PROJECT_ID).returns({
      ...
      discountRate: 0,
      ballot: ethers.constants.AddressZero,
      metadata: packFundingCycleMetadata({ reservedRate: 5000, allowMinting: 1 }),
    });
    ...
    await expect(
      jbController
        ...
        .mintTokensOf(
          ...
          MEMO,
          /*_preferClaimedTokens=*/ true,
          /* _useReservedRateOption=*/ NO_RESERVE_RATE,
        ),
    )
      .to.emit(jbController, 'MintTokens')
      .withArgs(
