...
describe('JBERC20PaymentTerminal::setFeeGauge(...)', function () {
  ...
  it('Should set the fee gauge and emit event if caller is terminal owner', async function () {
    const { terminalOwner, jbERC20PaymentTerminal, mockJbFeeGauge } = await setup();

    expect(await jbERC20PaymentTerminal.connect(terminalOwner).setFeeGauge(mockJbFeeGauge.address))
      .to.emit(jbERC20PaymentTerminal, 'SetFeeGauge')
  ...
  it("Can't set the fee gauge if caller is not the terminal owner", async function () {
    const { caller, jbERC20PaymentTerminal, mockJbFeeGauge } = await setup();

    await expect(
      jbERC20PaymentTerminal.connect(caller).setFeeGauge(mockJbFeeGauge.address),
