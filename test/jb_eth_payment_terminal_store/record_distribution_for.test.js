...
describe('JBETHPaymentTerminalStore::recordDistributionFor(...)', function () {
  ...
  it('Should record distribution', async function () {
    ...
    // pre-checks
    expect(await jbEthPaymentTerminalStore.usedDistributionLimitOf(PROJECT_ID, 1)).to.equal(0);
    ...
    // post-checks
    expect(await jbEthPaymentTerminalStore.usedDistributionLimitOf(PROJECT_ID, 1)).to.equal(AMOUNT);
  ...
  it(`Can't record distribution without terminal access`, async function () {
    const { terminal, addr, jbEthPaymentTerminalStore, CURRENCY_ETH } = await setup();
