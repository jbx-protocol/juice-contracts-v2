...
describe('JBDirectory::isTerminalDelegateOf(...)', function () {
  ...
  it('Should return true if delegate is set', async function () {
    const { caller, jbDirectory, terminal1, terminal2, terminal1Delegate, terminal2Delegate } =
      await setup();

    expect(
      await jbDirectory.connect(caller).isTerminalDelegateOf(PROJECT_ID, terminal1Delegate),
    ).to.be.true;

    expect(
      await jbDirectory.connect(caller).isTerminalDelegateOf(PROJECT_ID, terminal2Delegate),
    ).to.be.true;
  });
});
