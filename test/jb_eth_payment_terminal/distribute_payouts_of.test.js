...
describe.only('JBETHPaymentTerminal::distributePayoutsOf(...)', function () {
  ...
  it('Should distribute payout and emit event, without fee if project is the platform project', async function () {
  ...
  it('Should distribute payout minus fee, hold the fee in the contract and emit event', async function () {
    const { projectOwner, terminalOwner, caller, beneficiaryOne, beneficiaryTwo, jbEthPaymentTerminal, timestamp, mockJbEthPaymentTerminal, mockJbEthPaymentTerminalStore, mockJbSplitsStore } = await setup();
