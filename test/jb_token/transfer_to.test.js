...
describe('JBToken::transferFrom(...)', function () {
  ...
  it('Should transfer token and emit event if caller is owner', async function () {
    ...
    const transferTx = await jbToken.connect(addrs[3]).transferFrom(addrs[1].address, addrs[2].address, numTokens);
  ...
  it(`Can't transfer tokens if caller doesn't have approval`, async function () {
    ...
    await expect(
      jbToken.connect(addrs[1]).transferFrom(addrs[1].address, addrs[2].address, numTokens)
  ...
  it(`Can't transfer to zero address`, async function () {
    ...
    await jbToken.connect(addrs[1]).approve(addrs[3].address, numTokens);
    await expect(
      jbToken.connect(addrs[3]).transferFrom(addrs[1].address, ethers.constants.AddressZero, numTokens)
  ...
  it(`Can't transfer tokens if burn amount exceeds balance`, async function () {
    ...
    await jbToken.connect(addrs[1]).approve(addrs[3].address, numTokens);
    await expect(
      jbToken.connect(addrs[3]).transferFrom(addrs[1].address, addrs[2].address, numTokens)
