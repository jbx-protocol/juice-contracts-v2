    ...
    tests.success.forEach(function (successTest) {
      it(successTest.description, async function () {
        ...
        if (create) {
          await this.contract
            .connect(caller)
            .createFor(create.owner, create.handle, create.uri);
        }

        // Execute the transaction.
        const tx = await this.contract.connect(caller).createFor(owner, handle, uri);
    ...
    tests.failure.forEach(function (failureTest) {
      it(failureTest.description, async function () {
        ...
        if (create) {
          ...
          if (transfer) {
            await this.contract.connect(caller).transferHandleOf(1, transfer.to, transfer.handle);
          }
        }

        // Execute the transaction.
        await expect(
          this.contract.connect(caller).createFor(owner, handle, uri),
